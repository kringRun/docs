...
sidebarTitle: Integrate LangGraph with AutoGen, CrewAI, and other frameworks
---
...
* Enhanced features: Add [persistence](/oss/langgraph/persistence), [streaming](/langgraph-platform/streaming), [short and long-term memory](/oss/concepts/memory) and more to your AutoGen agents.
* Multi-agent systems: Build [multi-agent systems](/oss/langchain/multi-agent) where individual agents are built with different frameworks.
...
```python
from langchain.messages import convert_to_openai_messages
...
![LangGraph chatbot with one step: START routes to autogen, where call_autogen_agent sends the latest user message (with prior context) to the AutoGen agent.](/langgraph-platform/images/autogen-output.png)
...
Since we're leveraging LangGraph's [persistence](/oss/langgraph/persistence) features we can now continue the conversation using the same thread ID -- LangGraph will automatically pass previous history to the AutoGen agent:
