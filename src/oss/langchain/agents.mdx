...
:::python
1. A list of tools (LangChain @[`@tool`][@tool], callable, or `dict` that represents a built-in provider tool)
2. A configured @[`ToolNode`][ToolNode]
:::
:::js
...
Passing a list of tools to the agent will create a @[`ToolNode`][ToolNode] under the hood. This is the simplest way to set up a tool-calling agent:
...
Alternatively, you can create a @[`ToolNode`][ToolNode] directly and pass it to the agent. This allows you to customize the tool node's behavior, such as handling tool errors:
...
If an error occurs within @[`ToolNode`][ToolNode], the agent will return a `ToolMessage` to the model with the custom error message:
...
<Tip>
Learn more about how to use @[`ToolNode`][ToolNode] in [Tools](/oss/langchain/tools).
</Tip>
