...
:::python
1. A list of tools (LangChain @[`@tool`][@tool], callable, or `dict` that represents a builtin provider tool)
2. A configured @[`ToolNode`][ToolNode]
:::
:::js
1. A list of tools (created with @[`tool`][@tool] function, or object that represents a builtin provider tool)
2. A configured @[`ToolNode`][ToolNode]
:::
...
<Tip>
To learn more about error handling in @[`ToolNode`][ToolNode], see [ToolNode](/oss/langchain/tools#toolnode).
</Tip>
