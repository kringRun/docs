...
<Note>
This guide uses LangChain v1 syntax. If you're migrating from v0, see the [LangChain v1 migration guide](/oss/python/migrate/langchain-v1).
</Note>
...
You can use [`system_prompt` parameter](/oss/langchain/agents#system_prompt) to pass in a string to use as system prompt
...
You can use the [`@dynamic_prompt` decorator](/oss/langchain/agents#dynamic-prompt) from `langchain.agents.middleware` to create a function that dynamically generates the system prompt based on state, context, or memory
...
You can use middleware with the [`@before_model` decorator](/oss/langchain/agents#before-model) to update state before the model is invoked

Use cases:
- Filter out messages if message list is getting long, save filtered list in state and only use that
- Create a summary of conversation every N messages, save that in state (see [SummarizationMiddleware](/oss/langchain/agents#summarization-middleware))
...
You can use middleware with the [`@wrap_model_call` decorator](/oss/langchain/agents#wrap-model-call) to dynamically modify the tools passed to the model
...
You can use middleware with the [`@wrap_model_call` decorator](/oss/langchain/agents#wrap-model-call) to dynamically select which model to use
