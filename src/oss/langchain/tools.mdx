...
<Note>
This guide uses LangChain v1 syntax. If you're using v0, see the [migration guide](/oss/python/migrate/langchain-v1) to update your code.
</Note>
...
:::python
In v1, tool error handling is configured using middleware with the `@wrap_tool_call` decorator. This provides fine-grained control over how tool execution failures are handled within an agent's workflow.

For basic usage with `create_agent()`, pass tools as a list directly:
...
agent = create_agent(
    ...
    tools=[search_database, calculate],  # Pass tools as a list
    system_prompt="You are a helpful assistant."
)
```

For advanced error handling, use middleware with the `@wrap_tool_call` decorator:

```python wrap
from langchain.agents import create_agent
from langchain.agents.middleware import AgentMiddleware, wrap_tool_call, ToolCallRequest, ToolCallHandler
from langchain.tools import tool
...
class ErrorHandlingMiddleware(AgentMiddleware):
    @wrap_tool_call
    def handle_errors(self, request: ToolCallRequest, handler: ToolCallHandler) -> ToolMessage:
        try:
            return handler(request)
        except ValueError as e:
            return ToolMessage(
                content=f"Error: {e}. Please rephrase your request.",
...
The `@wrap_tool_call` decorator allows you to intercept tool calls and handle errors gracefully, returning `ToolMessage` objects that the agent can use to recover and retry.

<Note>
In v0, `ToolNode` was passed as a parameter to `create_react_agent`. In v1, `create_agent()` accepts tools as a list directly, and error handling is configured via middleware. See the [migration guide](/oss/python/migrate/langchain-v1#tools) for more details.
</Note>
:::

:::js
`ToolNode` is a prebuilt LangChain component that handles tool calls within an agent's workflow. It works seamlessly with `createAgent()`, offering advanced tool execution control, built-in parallelism, and error handling.
...
<Note>
    We recommend that you familiarize yourself with `createAgent()` before covering this section. [Read more about agents](/oss/langchain/agents).
</Note>
...
const sendEmail = tool(
  ({ to, subject, body }) => `Email sent to ${to}`,
  {
    name: "send_email",
...
:::python
    <Accordion title="Accessing agent state inside a tool">
        ...
        # Access custom state fields - requires middleware with state_schema
        ...
        class UserPreferencesMiddleware(AgentMiddleware[CustomState]):
        ...
        # Create agent with middleware that defines custom state
        agent = create_agent(
            model="anthropic:claude-sonnet-4-5-20250929",
        ...
        <Note>
            Custom state fields must be defined in middleware using the `state_schema` attribute, which must be a `TypedDict`. See the [migration guide](/oss/python/migrate/langchain-v1#custom-state) for more details.
        </Note>
    </Accordion>
