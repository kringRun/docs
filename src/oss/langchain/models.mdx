...
<Note>
This guide has been updated for LangChain v1. If you're upgrading from v0, see the [migration guide](/oss/python/migrate/langchain-v1) for breaking changes and migration patterns.
</Note>
...
1. **With agents** - Models can be dynamically specified when creating an agent. See the [agents](/oss/langchain/agents#model) guide for more details on how to do this.
...
:::python
The easiest way to get started with a standalone model in LangChain is to use `init_chat_model` to initialize one from a [provider](/oss/integrations/providers) of your choice (examples below):
...
:::
:::js
The easiest way to get started with a standalone model in LangChain is to use `initChatModel` to initialize one from a [provider](/oss/integrations/providers) of your choice (examples below):
...
:::
<Info>
    To find all the parameters supported by a given chat model, head to the [integrations](/oss/integrations/providers) page.
</Info>
...
<Note>
    **Key considerations for structured outputs:**
    ...
    - **Agent compatibility**: When using models with [agents](/oss/langchain/agents), pre-bound models (created with `bind_tools()` or `with_structured_output()`) are not supported. Pass the base model to `create_agent` instead.
</Note>
:::
