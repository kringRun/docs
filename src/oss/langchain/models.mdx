...
:::python
The easiest way to get started with a model in LangChain is to use `init_chat_model` to initialize one from a [provider](/oss/integrations/providers) of your choice (examples below):
...
:::
:::js
The easiest way to get started with a model in LangChain is to use `initChatModel` to initialize one from a [provider](/oss/integrations/providers) of your choice (examples below):
...
<AccordionGroup>
    <Accordion title="Tool execution loop" icon="arrow-rotate-right">
        ...
        LangChain includes [agent](/oss/langchain/agents) abstractions that handle this orchestration for you. Below, we illustrate a simple example:
