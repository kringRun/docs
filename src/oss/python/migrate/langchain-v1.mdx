...
| [Tools](#tools) | `ToolNode` replaced by simple list, new error handling |
| [Structured output](#structured-output) | `PromptedOutput` removed, use `ToolStrategy`/`ProviderStrategy` |
| [Streaming node name](#streaming-node-name-rename) | Node name changed from `"agent"` to `"model"` |
| [Runtime context](#runtime-context) | New `context` parameter replaces `config["configurable"]` |

### Quick reference

| Pattern | V0 (`create_react_agent`) | V1 (`create_agent`) |
|---------|---------------------------|---------------------|
| [Import path](#import-path) | `langgraph.prebuilt` | `langchain.agents` |
| [Static prompt](#prompts) | `prompt="..."` | `system_prompt="..."` |
| [Dynamic prompt](#prompts) | `prompt=callable` | `middleware=[@modify_model_request]` |
| [Pre-processing](#pre-model-hook) | `pre_model_hook=fn` | `middleware` with `before_model` |
| [Post-processing](#post-model-hook) | `post_model_hook=fn` | `middleware` with `after_model` |
| [Custom state](#custom-state) | `state_schema=CustomState` | `middleware` with `state_schema` |
| [State types](#custom-state) | Pydantic/dataclass/TypedDict | `TypedDict` only |
| [Dynamic model](#model) | `model=callable` | `middleware=[CustomMiddleware]` |
| [Pre-bound models](#model) | `model.bind_tools([...])` | Not supported with structured output |
| [Tool list](#tools) | `tools=ToolNode([...])` | `tools=[...]` |
| [Tool errors](#tools) | - | `middleware` with error handling |
| [Structured output](#structured-output) | `response_format=PromptedOutput` | `response_format=ToolStrategy/ProviderStrategy` |
| [Streaming node](#streaming-node-name-rename) | Node name: `"agent"` | Node name: `"model"` |
| [Runtime context](#runtime-context) | `config={"configurable": {...}}` | `context={...}` |
...
Dynamic model selection allows you to choose different models based on runtime context (e.g., task complexity, cost constraints, or user preferences).
