...
<Accordion title="Full example">
    ...
    def approval_node(state: ApprovalState):
        # Emit payload for the caller; shows up under result["__interrupt__"]
        approved = interrupt({
    ...
    # Persist checkpoints so another worker (or later process) can resume safely
    checkpointer = SqliteSaver(sqlite3.connect("approval.db"))
    graph = builder.compile(checkpointer=checkpointer)

    # thread_id is your persistent pointer back to the saved checkpoint
    config = {"configurable": {"thread_id": "workflow-42"}}
    initial = graph.invoke({"input": "transfer $500"}, config=config)
    print(initial["__interrupt__"])  # -> [Interrupt(value={'question': ..., 'input': ...})]

    # Later, resume with the same thread_id; the payload becomes the return value of interrupt()
    graph.invoke(Command(resume=True), config=config)
    ```
    :::
    ...
    const builder = new StateGraph(State)
      .addNode("approval", async (state) => {
        // Emit payload for the caller; appears in result.__interrupt__
    ...
    // thread_id is the stable pointer to the saved checkpoint
    const config = { configurable: { thread_id: "workflow-42" } };
    const initial = await graph.invoke({ input: "transfer $500" }, config);
    console.log(initial.__interrupt__); // -> [{ value: { question: ..., input: ... } }]

    // Resume with the user's answer; the value is returned from interrupt() in the node
    await graph.invoke(new Command({ resume: true }), config);
    ```
    :::
