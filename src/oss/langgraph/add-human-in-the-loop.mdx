...
You can use [LangGraph Studio](/langsmith/langgraph-studio) to debug your graph. You can set static breakpoints in the UI and then run the graph. You can also use the UI to inspect the graph state at any point in the execution.

![image](/oss/images/static-interrupt.png)

LangGraph Studio is free with [locally deployed applications](/oss/langgraph/local-server) using `langgraph dev`.
:::
