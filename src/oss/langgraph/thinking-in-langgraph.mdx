...
<CardGroup cols={2}>
  <Card title="LLM Steps" icon="brain">
  ...
  <Card title="User Input Steps" icon="user">
...
| Error Type | Strategy | When to Use |
|------------|----------|-------------|
| **Transient errors** (network issues, rate limits) | Retry policy | Temporary failures that usually resolve on retry |
| **LLM-recoverable errors** (tool failures, parsing issues) | Store error in state and loop back | LLM can adjust its approach based on the error |
| **User-fixable errors** (missing credentials, unclear instructions) | Pause with `interrupt()` | Need human input to proceed |
| **Unexpected errors** | Let them bubble up | Unknown issues that need debugging |
...
```python
def draft_response(state: State) -> Command[Literal["continue"]]:
    if not state.get('customer_account_id'):
        user_input = interrupt({
            "error": "Customer account ID missing",
            "request": "Please provide the customer's account ID to look up their history"
        })
        return Command(
            update={"customer_account_id": user_input['account_id']},
            goto="continue"
        )
```
...
You might wonder: why not combine Read Email and Classify Intent into one node? Or why separate Doc Search from Draft Reply? The answer involves trade-offs between resilience, observability, and performance.

**The performance consideration:** LangGraph serializes and deserializes state at each node boundary, creating a checkpoint. This overhead is typically small (milliseconds), but it does exist. For most applications, this is negligible compared to LLM and API call latencies. However, if you're building an extremely latency-sensitive application, you might choose to combine nodes to reduce the number of checkpoints.
...
**A different valid approach:** You could combine Read Email and Classify Intent into a single node to reduce one checkpoint. The performance gain would be small (one less serialization/deserialization), but you'd lose the ability to inspect the raw email before classification and would repeat both operations on any failure in that node. For most applications, the observability and debugging benefits of separate nodes outweigh the minimal performance cost.

**Tuning durability vs. performance:** If checkpointing overhead becomes a concern for your application, you can adjust the [durability mode](/oss/langgraph/durable-execution#durability-modes) when executing your graph. The default `"async"` mode provides good performance while maintaining durability. For maximum performance in long-running graphs where you don't need mid-execution recovery, use `"exit"` mode (checkpoints only at completion). For critical workflows where you need guaranteed checkpoint writes, use `"sync"` mode.

### Where to go from here

This was an introduction to thinking about building agents with LangGraph. You can extend this foundation with:

- **Enhanced human-in-the-loop patterns**: Learn how to add [tool approval before execution, batch approval, and other patterns](/oss/langgraph/add-human-in-the-loop)
- **Create subgraphs** for [complex multi-step operations](/oss/langgraph/use-subgraphs)
- **Add streaming** to [show real-time progress to users](/oss/langgraph/streaming)
- **Integrate more tools** for web search, database queries, and API calls
- **Implement retry logic** with exponential backoff for failed operations
- **Add observability** with [LangSmith for debugging and monitoring](/oss/langgraph/observability)
