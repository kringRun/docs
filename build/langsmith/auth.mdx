...
In LangSmith, authentication is handled by your [`@auth.authenticate`](/langsmith/python-sdk#langgraph_sdk.auth.Auth.authenticate) handler, and authorization is handled by your [`@auth.on`](/langsmith/python-sdk#langgraph_sdk.auth.Auth.on) handlers.
...
Your [`@auth.authenticate`](/langsmith/python-sdk#langgraph_sdk.auth.Auth.authenticate) handler in LangGraph handles steps 4-6, while your [`@auth.on`](/langsmith/python-sdk#langgraph_sdk.auth.Auth.on) handlers implement step 7.

## Authentication

Authentication in LangGraph runs as middleware on every request. Your [`@auth.authenticate`](/langsmith/python-sdk#langgraph_sdk.auth.Auth.authenticate) handler receives request information and should:
