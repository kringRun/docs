...
To create a new thread, use the [LangGraph SDK](/langsmith/sdk) `create` method. See the [Python](/langsmith/python-sdk#langgraph_sdk.client.ThreadsClient.create) and [JS](/langsmith/js-ts-sdk#create_3) SDK reference docs for more information.
...
Alternatively, if you already have a thread in your application whose state you wish to copy, you can use the `copy` method. This will create an independent thread whose history is identical to the original thread at the time of the operation. See the [Python](/langsmith/python-sdk#langgraph_sdk.client.ThreadsClient.copy) and [JS](/langsmith/js-ts-sdk#copy) SDK reference docs for more information.
...
To list threads, use the [LangGraph SDK](/langsmith/sdk) `search` method. This will list the threads in the application that match the provided filters. See the [Python](/langsmith/python-sdk#langgraph_sdk.client.ThreadsClient.search) and [JS](/langsmith/js-ts-sdk#search_2) SDK reference docs for more information.

#### Filter by thread status

Use the `status` field to filter threads based on their status. Supported values are `idle`, `busy`, `interrupted`, and `error`. See [here](/langsmith/python-sdk?h=thread+status#langgraph_sdk.auth.types.ThreadStatus) for information on each status. For example, to view `idle` threads:
...
### LangSmith Platform UI

You can also view threads in a deployment via the LangSmith Platform UI.
...
To view a thread's history, use the `get_history` method. This returns a list of every state the thread experienced. For more information see the [Python](/langsmith/python-sdk?h=thread+status#langgraph_sdk.client.ThreadsClient.get_history) and [JS](/langsmith/js-ts-sdk#gethistory) reference docs.
...
Select a thread to inspect its current state. To view its full history and for further debugging, open the thread in the [Debugger](/langsmith/debugger).
