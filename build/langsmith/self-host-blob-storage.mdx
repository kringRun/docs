...
As of LangSmith helm chart version **0.11.24**, we support passing a KMS encryption key header, as well as enforcing a KMS key while writing to a bucket, by passing a key ARN. To enable, set the following values in your helm chart:
```yaml Helm
config:
  blobStorage:
