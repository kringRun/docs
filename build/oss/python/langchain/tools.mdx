...
<Note>
This guide uses LangChain v1 syntax. If you're using v0, see the [migration guide](/oss/python/python/migrate/langchain-v1) to update your code.
</Note>
...
<Tip>To see examples of how to use tools with agents, see [Agents](/oss/python/langchain/agents).</Tip>
...
<Note>
In v0, `ToolNode` was passed as a parameter to `create_react_agent`. In v1, `create_agent()` accepts tools as a list directly, and error handling is configured via middleware. See the [migration guide](/oss/python/python/migrate/langchain-v1#tools) for more details.
</Note>
...
<AccordionGroup>
    <Accordion title="Accessing agent state inside a tool">
        ...
        <Note>
            Custom state fields must be defined in middleware using the `state_schema` attribute, which must be a `TypedDict`. See the [migration guide](/oss/python/python/migrate/langchain-v1#custom-state) for more details.
        </Note>
    </Accordion>
