...
To trim message history in an agent, use middleware with the [`@before_model`][before_model] decorator and the [`trim_messages`](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.utils.trim_messages.html) function:
...
from langgraph.checkpoint.memory import InMemorySaver
from langchain.agents import create_agent
from langchain.agents.middleware import AgentMiddleware, before_model, ModelRequest
from langchain_core.runnables import RunnableConfig

class TrimMessagesMiddleware(AgentMiddleware):
    """
    This middleware will be called prior to every llm call to prepare the messages for the llm.
    """

    @before_model
    def trim_messages_hook(self, request: ModelRequest) -> ModelRequest:
        ...
        return request.replace(messages=trimmed_messages)
...
class DeleteMessagesMiddleware(AgentMiddleware):

    @after_model
    def delete_messages(self, response: ModelResponse) -> ModelResponse:
        messages = response.state["messages"]
        if len(messages) > 2:
            # remove the earliest two messages
            return response.update_state({"messages": [RemoveMessage(id=m.id) for m in messages[:2]]})
        return response
...
agent = create_agent(
    ...
    middleware=[DeleteMessagesMiddleware()],
    checkpointer=InMemorySaver(),
)
...
To summarize message history in an agent, use the built-in [`SummarizationMiddleware`](https://python.langchain.com/api_reference/langchain/agents/langchain.agents.middleware.SummarizationMiddleware.html):

```python {highlight={2,9-12}}
from langchain.agents import create_agent
...
class ValidateResponseMiddleware(AgentMiddleware):

    @after_model
    def validate_response(self, response: ModelResponse) -> ModelResponse:
        """Confirm the response doesn't have any content that is in the stop words list."""
        last_message = response.state["messages"][-1]
        if any(word in last_message.content for word in STOP_WORDS):
            return response.update_state({"messages": [RemoveMessage(id=last_message.id)]})
        return response

agent = create_agent(
    model="openai:gpt-5-nano",
    tools=[],
    middleware=[ValidateResponseMiddleware()],
    checkpointer=InMemorySaver(),
)
```
