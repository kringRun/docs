...
class DynamicPromptMiddleware(AgentMiddleware):
    def wrap_model_call(self, request: ModelRequest, handler: ModelRequestHandler):
        ...
        system_prompt = f"You are a helpful assistant. Address the user as {user_name}."  # [!code highlight]
        modified_request = request.replace(system_prompt=system_prompt)  # [!code highlight]
