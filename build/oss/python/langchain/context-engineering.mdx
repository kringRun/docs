...
<Note>
This guide uses LangChain v1 syntax. If you're migrating from v0, see the [LangChain v1 migration guide](/oss/python/python/migrate/langchain-v1).
</Note>
...
You can use [`system_prompt` parameter](/oss/python/langchain/agents#system_prompt) to pass in a string to use as system prompt
...
You can use the [`@dynamic_prompt` decorator](/oss/python/langchain/agents#dynamic-prompt) from `langchain.agents.middleware` to create a function that dynamically generates the system prompt based on state, context, or memory

Use cases:
- Reinforce instructions by dynamically adjusting the system message based on conversation state
- Personalize the system prompt with information in session context, long term memory, or runtime context
- Use `user_id` from context to look up user profile and customize the system prompt
...
You can use middleware with the [`@before_model` decorator](/oss/python/langchain/agents#before-model) to update state before the model is invoked

Use cases:
- Filter out messages if message list is getting long, save filtered list in state and only use that
- Create a summary of conversation every N messages, save that in state (see [SummarizationMiddleware](/oss/python/langchain/agents#summarization-middleware))

### Access to runtime configuration in tools

You can use `get_runtime` to [access runtime configuration](/oss/python/langchain/tools#accessing-runtime-context-inside-a-tool) in tools
...
You can add an argument with InjectedState to tools to access [session context in tools](/oss/python/langchain/short-term-memory#read-short-term-memory-in-a-tool)
...
You can use `get_store` to [access long term memory in tools](/oss/python/langchain/long-term-memory#read-long-term-memory-in-tools)
...
You can [return state updates](/oss/python/langchain/short-term-memory#write-short-term-memory-from-tools) with Command from tools
...
You can use `get_store` to access long term memory and then [update it inside tools](/oss/python/langchain/long-term-memory#write-long-term-memory-from-tools)
...
You can use middleware with the [`@wrap_model_call` decorator](/oss/python/langchain/agents#wrap-model-call) to dynamically modify the tools passed to the model
...
You can use middleware with the [`@wrap_model_call` decorator](/oss/python/langchain/agents#wrap-model-call) to dynamically select which model to use
