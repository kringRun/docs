...
`create_agent()` builds a **graph**-based agent runtime using [LangGraph](/oss/python/langgraph/overview). A graph consists of nodes (steps) and edges (connections) that define how your agent processes information. The agent moves through this graph, executing nodes like the model node (which calls the model), the tools node (which executes tools), or pre/post model hook nodes.
...
Model instances give you complete control over configuration. Use them when you need to set specific [parameters](/oss/python/langchain/models#parameters) like temperature, max tokens, timeouts, base URLs, and other provider-specific settings. Refer to the [reference](/oss/python/integrations/providers/all_providers) to see available params and methods on your model.
...
To use a dynamic model, you need to provide a function that receives the graph state and runtime and returns an instance of a chat model. The returned chat model needs to have the tools bound to it using `.bind_tools(tools)`, where `tools` is a subset of the `tools` parameter.
...
1. A list of tools (LangChain [`@tool`](https://python.langchain.com/api_reference/core/tools/langchain_core.tools.convert.tool.html), callable, or `dict` that represents a built-in provider tool)
2. A configured [`ToolNode`](https://langchain-ai.github.io/langgraph/reference/agents/#langgraph.prebuilt.tool_node.ToolNode)



#### Pass a list of tools

Passing a list of tools to the agent will create a [`ToolNode`](https://langchain-ai.github.io/langgraph/reference/agents/#langgraph.prebuilt.tool_node.ToolNode) under the hood. This is the simplest way to set up a tool-calling agent:
...
Alternatively, you can create a [`ToolNode`](https://langchain-ai.github.io/langgraph/reference/agents/#langgraph.prebuilt.tool_node.ToolNode) directly and pass it to the agent. This allows you to customize the tool node's behavior, such as handling tool errors:
...
If an error occurs within [`ToolNode`](https://langchain-ai.github.io/langgraph/reference/agents/#langgraph.prebuilt.tool_node.ToolNode), the agent will return a `ToolMessage` to the model with the custom error message:
...
<Tip>
Learn more about how to use [`ToolNode`](https://langchain-ai.github.io/langgraph/reference/agents/#langgraph.prebuilt.tool_node.ToolNode) in [Tools](/oss/python/langchain/tools).
</Tip>
