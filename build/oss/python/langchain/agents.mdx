...
<Info>
This documentation covers LangChain v1. If you're upgrading from v0, see the [v1 migration guide](/oss/python/python/migrate/langchain-v1) for changes to agent APIs.
</Info>
...
Pass a list of tools to the agent. The agent will create a [`ToolNode`](https://langchain-ai.github.io/langgraph/reference/agents/#langgraph.prebuilt.tool_node.ToolNode) under the hood to execute them:
...
<Tip>
Learn more about tools in [Tools](/oss/python/langchain/tools).
</Tip>
...
<Note>
In v1, custom state schemas **must** be `TypedDict` types. Pydantic models and dataclasses are no longer supported. See the [v1 migration guide](/oss/python/python/migrate/langchain-v1#state-type-restrictions) for more details.
</Note>
