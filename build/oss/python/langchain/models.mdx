...
<Note>
This guide has been updated for LangChain v1. If you're upgrading from v0, see the [migration guide](/oss/python/python/migrate/langchain-v1) for breaking changes and migration patterns.
</Note>
...
<Info>
    For provider-specific integration information and capabilities, see the provider's [integration page](/oss/python/integrations/providers).
</Info>
...
1. **With agents** - Models can be dynamically specified when creating an agent. See the [agents](/oss/python/langchain/agents#model) guide for more details on how to do this.
...
The easiest way to get started with a standalone model in LangChain is to use `init_chat_model` to initialize one from a [provider](/oss/python/integrations/providers) of your choice (examples below):

<ChatModelTabsPy />
```python
response = model.invoke("Why do parrots talk?")
```
See [`init_chat_model`](https://python.langchain.com/api_reference/langchain/chat_models/langchain.chat_models.base.init_chat_model.html) for more detail.
...
<Info>
    In addition to chat models, LangChain provides support for other adjacent technologies, such as embedding models and vector stores. See the [integrations page](/oss/python/integrations/providers) for details.
</Info>
...
<Info>
    To find all the parameters supported by a given chat model, head to the [integrations](/oss/python/integrations/providers) page.
</Info>
...
<Tip>
    ...
    A full list of async methods can be found in the [reference docs](https://python.langchain.com/api_reference/).
</Tip>


### Invoke

The most straightforward way to call a model is to use [`invoke()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.invoke) with a single message or a list of messages.
...
Calling [`stream()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.stream) returns an <Tooltip tip="An object that progressively provides access to each item of a collection, in order.">iterator</Tooltip> that yields output chunks as they are produced. You can use a loop to process each chunk in real-time:
...
As opposed to `invoke()`, which returns a single [`AIMessage`](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.ai.AIMessage.html) after the model has finished generating its full response, `stream()` returns multiple [`AIMessageChunk`](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.ai.AIMessageChunk.html) objects, each containing a portion of the output text. Importantly, each chunk in a stream is designed to be gathered into a full message via summation:
...
<Accordion title="Advanced streaming topics">
    <Accordion title='"Auto-streaming" chat models'>
        ...
        Callback events allow LangGraph `stream()` and [`astream_events()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.astream_events) to surface the chat model's output in real-time.
    ...
    <Accordion title="Streaming events">
        ...
        See the [`astream_events()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.astream_events)
...
By default, [`batch()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.batch) will only return the final output for the entire batch. If you want to receive the output for each individual input as it finishes generating, you can stream results with [`batch_as_completed()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.batch_as_completed):
...
<Tip>
    ...
    See the [`RunnableConfig`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.config.RunnableConfig.html) reference for a full list of supported attributes.
</Tip>
...
To make tools that you have defined available for use by a model, you must bind them using [`bind_tools()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.bind_tools). In subsequent invocations, the model can choose to call any of the bound tools as needed.




Some model providers offer built-in tools that can be enabled via model parameters. Check the respective [provider reference](/oss/python/integrations/providers) for details.
...
<Note>
    **Key considerations for structured outputs:**
    ...
    - **Agent compatibility**: When using models with [agents](/oss/python/langchain/agents), pre-bound models (created with `bind_tools()` or `with_structured_output()`) are not supported. Pass the base model to `create_agent` instead.
</Note>
...
LangChain supports all major model providers, including OpenAI, Anthropic, Google, Azure, AWS Bedrock, and more. Each provider offers a variety of models with different capabilities. For a full list of supported models in LangChain, see the [integrations page](/oss/python/integrations/providers).
...
Certain models can process and return non-textual data such as images, audio, and video. You can pass non-textual data to a model by providing [content blocks](/oss/python/langchain/messages#content).
...
See the [integrations page](/oss/python/integrations/providers) for details on specific providers.
...
For details, see the relevant chat model in the [integrations page](/oss/python/integrations/providers).
...
[Ollama](/oss/python/integrations/chat/ollama) is one of the easiest ways to run models locally. See the full list of local integrations on the [integrations page](/oss/python/integrations/providers).
...
<Accordion title="Base URL" icon="link">
    ...
    <Note>
        When using direct chat model class instantiation, the parameter name may vary by provider. Check the respective [reference](/oss/python/integrations/providers) for details.
    </Note>
</Accordion>
...
<Note>
    Proxy support varies by integration. Check the specific model provider's [reference](/oss/python/integrations/providers) for proxy configuration options.
</Note>
...
A number of model providers return token usage information as part of the invocation response. When available, this information will be included on the `AIMessage` objects produced by the corresponding model. For more details, see the [messages](/oss/python/langchain/messages) guide.
...
When invoking a model, you can pass additional configuration through the `config` parameter using a [`RunnableConfig`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.config.RunnableConfig.html) dictionary. This provides run-time control over execution behavior, callbacks, and metadata tracking.
...
For more information on all supported `RunnableConfig` attributes, see the [`RunnableConfig`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.config.RunnableConfig.html) reference.
