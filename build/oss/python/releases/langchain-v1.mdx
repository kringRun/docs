...
LangChain v1 introduces simplified import paths for commonly-used abstractions, making it easier to get started:

### Tools

Tool-related imports are now available from `langchain.tools`:
...
Previously, these were only available from `langchain_core.tools`.

### Messages

All message types are now available from `langchain.messages`:
...
Previously, these were only available from `langchain_core.messages`.

These new import paths provide convenient access to the most commonly-used LangChain abstractions while maintaining backward compatibility with the original `langchain_core` imports.
