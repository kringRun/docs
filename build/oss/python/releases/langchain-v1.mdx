...
|--------|---------|-------------|
| `langchain.agents` | Agent creation and orchestration | `create_agent`, `AgentState` |
| `langchain.chat_models` | Chat model initialization | `init_chat_model`, `BaseChatModel` |
| `langchain.messages` | Message types for LLM interactions | `AIMessage`, `HumanMessage`, `SystemMessage`, `ToolMessage`, content blocks, `trim_messages` |
| `langchain.tools` | Tool creation and management | `tool`, `BaseTool`, `ToolNode`, injection helpers |
| `langchain.embeddings` | Embedding models | `Embeddings`, `init_embeddings`, `CacheBackedEmbeddings` |
| `langchain.documents` | Document representation | `Document` |
| `langchain.storage` | Key-value storage for caching | `InMemoryStore`, `EncoderBackedStore` |
| `langchain.rate_limiters` | Rate limiters | `BaseRateLimiter`, `InMemoryRateLimiter` |
