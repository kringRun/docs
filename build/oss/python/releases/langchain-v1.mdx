...
<CardGroup cols={1}>
    ...
    <Card title="LangChain Classic" icon="arrows-turn-right" href="#langchain-classic" arrow>
...
For more information, see [Agents](/oss/python/python/langchain/agents).
### Middleware
...
For more information, see [the complete middleware guide](/oss/python/python/langchain/middleware).
...
For more information, see our guide on [content blocks](/oss/python/langchain/messages#content)

---

## Simplified import paths

LangChain v1 introduces convenient import paths for commonly used abstractions, making it easier to access tools and messages without importing from `langchain_core`:

### Tools

Import tools and tool-related classes directly from `langchain.tools`:

```python
from langchain.tools import tool, ToolNode, InjectedState, InjectedStore, InjectedToolCallId
```

Previously, these were imported from `langchain_core.tools`.

### Messages

Import message types directly from `langchain.messages`:

```python
from langchain.messages import AIMessage, HumanMessage, SystemMessage, ToolMessage
```

Previously, these were imported from `langchain_core.messages`.

These new import paths provide easier access to common abstractions while maintaining backward compatibility with existing `langchain_core` imports.
...
<CardGroup cols={3}>
    ...
    <Card title="Migration guide" icon="arrow-right-arrow-left" href="/oss/python/python/migrate/langchain-v1" arrow>
