...
**Tuning durability vs. performance:** If checkpointing overhead becomes a concern for your application, you can adjust the [durability mode](/oss/python/langgraph/durable-execution#durability-modes) when executing your graph. The default `"async"` mode provides good performance while maintaining durability. For maximum performance in long-running graphs where you don't need mid-execution recovery, use `"exit"` mode (checkpoints only at completion). For critical workflows where you need guaranteed checkpoint writes, use `"sync"` mode.

### Where to go from here

This was an introduction to thinking about building agents with LangGraph. You can extend this foundation with:

- **Enhanced human-in-the-loop patterns**: Learn how to add [tool approval before execution, batch approval, and other patterns](/oss/python/langgraph/add-human-in-the-loop)
- **Create subgraphs** for [complex multi-step operations](/oss/python/langgraph/use-subgraphs)
- **Add streaming** to [show real-time progress to users](/oss/python/langgraph/streaming)
- **Integrate more tools** for web search, database queries, and API calls
- **Implement retry logic** with exponential backoff for failed operations
- **Add observability** with [LangSmith for debugging and monitoring](/oss/python/langgraph/observability)
