...
| `langchain.tools` | `tool`, `BaseTool`, `ToolNode`, injection helpers | Re-exported from `langchain-core` |
| `langchain.chat_models` | `init_chat_model`, `BaseChatModel` | Unified model initialization |
| `langchain.embeddings` | `Embeddings`, `init_embeddings`, `CacheBackedEmbeddings` | Embedding models and caching |
| `langchain.documents` | `Document` | Re-exported from `langchain-core` |
| `langchain.storage` | `InMemoryStore`, `EncoderBackedStore`, etc. | Key-value storage for caching |
