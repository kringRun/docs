...
LangChain v1 introduces simplified import paths for commonly-used abstractions. While the original `langchain_core` imports still work, we recommend migrating to the new paths for better ergonomics.
...
```
```python V0 (old)
from langchain_core.tools import tool, ToolNode, InjectedState, InjectedStore, InjectedToolCallId
...
<Note>
The original `langchain_core.tools` and `langchain_core.messages` import paths continue to work for backward compatibility, but the new simplified paths are recommended for new code.
</Note>
