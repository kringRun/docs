...
* How interruptions are resumed in [human-in-the-loop](/oss/javascript/langgraph/add-human-in-the-loop) workflows
...
They also determine how execution steps are [streamed](/oss/javascript/langgraph/streaming), and how your application is visualized
...
If you are using [subgraphs](/oss/javascript/concepts/subgraphs), you might want to navigate from a node within a subgraph to a different subgraph (i.e. a different node in the parent graph). To do so, you can specify `graph=Command.PARENT` in `Command`:
